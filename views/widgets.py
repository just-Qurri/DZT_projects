...
class Card(ttk.Frame):
    ...
    def __init__(self, parent, title="", width=None, padding=AppStyles.PADDING_LG, **kwargs):
        ...
        if title:
            ...
            # Устанавливаем ширину, если передана
...
class RadioGroup(ttk.Frame):
    def __init__(self, parent, options, width=None, **kwargs):
        super().__init__(parent, style='RadioGroup.TFrame', **kwargs)

        self._value_var = StringVar()
        self._value_var.set(options[0][1])
        ...
        for text, value in options:
            ...
            if width is not None:
                rb.config(width=width)

            # ДОБАВИТЬ fill=X для растяжения кнопок
